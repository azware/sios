...
describe("API baseline", () => {
  ...
  it("PUT /api/payments/:id should return 404 when not found", async () => {
    const adminToken = jwt.sign({ id: 1004, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 1004,
      username: "admin_payment_notfound",
      email: "admin_payment_notfound@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
    prismaMock.payment.findUnique.mockResolvedValueOnce(null);

    const res = await request(app).put("/api/payments/999").set("Authorization", `Bearer ${adminToken}`).send({
      status: "PAID",
    });
    expect(res.status).toBe(404);
  });

  it("DELETE /api/payments/:id should return 403 for teacher role", async () => {
    const teacherToken = jwt.sign({ id: 1005, role: "TEACHER" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 1005,
      username: "teacher_payment_delete",
      email: "teacher_payment_delete@sios.local",
      role: "TEACHER",
      password: "hashed",
    });

    const res = await request(app).delete("/api/payments/50").set("Authorization", `Bearer ${teacherToken}`);
    expect(res.status).toBe(403);
  });
  ...
  it("PUT /api/grades/:id should return 404 when not found", async () => {
    const adminToken = jwt.sign({ id: 1105, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 1105,
      username: "admin_grade_notfound",
      email: "admin_grade_notfound@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
    prismaMock.grade.findUnique.mockResolvedValueOnce(null);

    const res = await request(app).put("/api/grades/999").set("Authorization", `Bearer ${adminToken}`).send({
      score: 75,
    });
    expect(res.status).toBe(404);
  });

  it("DELETE /api/grades/:id should return 403 for teacher modifying other teacher's grade", async () => {
    const teacherToken = jwt.sign({ id: 1106, role: "TEACHER" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 1106,
      username: "teacher_grade_delete",
      email: "teacher_grade_delete@sios.local",
      role: "TEACHER",
      password: "hashed",
    });
    prismaMock.teacher.findUnique.mockResolvedValueOnce({
    ...
    const res = await request(app).delete("/api/grades/61").set("Authorization", `Bearer ${teacherToken}`);
    expect(res.status).toBe(403);
  });
  ...
  it("GET /api/classes/:id should return 200 for admin", async () => {
    const adminToken = jwt.sign({ id: 2001, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 2001,
      username: "admin_class_detail",
      email: "admin_class_detail@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
    ...
    const res = await request(app).get("/api/classes/501").set("Authorization", `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.id).toBe(501);
  });

  it("GET /api/classes/:id should return 404 when not found", async () => {
    const adminToken = jwt.sign({ id: 2002, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 2002,
      username: "admin_class_notfound",
      email: "admin_class_notfound@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
  ...
  it("GET /api/schools/:id should return 200 for admin", async () => {
    const adminToken = jwt.sign({ id: 2003, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 2003,
      username: "admin_school_detail",
      email: "admin_school_detail@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
    prismaMock.school.findUnique.mockResolvedValueOnce({
      id: 701,
      name: "Sekolah A",
      classes: [],
      teachers: [],
    ...
    const res = await request(app).get("/api/schools/701").set("Authorization", `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.id).toBe(701);
  });

  it("GET /api/schools/:id should return 404 when not found", async () => {
    const adminToken = jwt.sign({ id: 2004, role: "ADMIN" }, process.env.JWT_SECRET || "secret");
    prismaMock.user.findUnique.mockResolvedValueOnce({
      id: 2004,
      username: "admin_school_notfound",
      email: "admin_school_notfound@sios.local",
      role: "ADMIN",
      password: "hashed",
    });
    prismaMock.school.findUnique.mockResolvedValueOnce(null);

    const res = await request(app).get("/api/schools/999").set("Authorization", `Bearer ${adminToken}`);
    expect(res.status).toBe(404);
  });
